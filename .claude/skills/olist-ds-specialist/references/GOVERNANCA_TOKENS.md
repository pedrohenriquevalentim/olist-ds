...
### 3.3 `font-weight-*` — ⚠️ bug ativo, leia antes de usar

A escala primitiva tem 7 pesos, com aliases semânticos de nome mais curto:

| Peso | Primitivo | Alias semântico | Status |
|---|---|---|---|
| 200 | `extra-light` | `xlight` | ✅ resolve corretamente |
| 300 | `light` | `light` | ❌ **quebrado** (referência circular) |
| 400 | `regular` | `regular` | ❌ **quebrado** (referência circular) |
| 500 | `medium` | `medium` | ❌ **quebrado** (referência circular) |
| 600 | `semibold` | `sbold` | ✅ resolve corretamente |
| 700 | `bold` | `bold` | ❌ **quebrado** (referência circular) |
| 800 | `extra-bold` | `xbold` | ✅ resolve corretamente |

- **purpose:** peso de fonte
- **useWhen:** hoje, só `--font-weight-xlight`, `--font-weight-sbold` e
  `--font-weight-xbold` são seguros para uso direto
- **doNotUseWhen:** não use `--font-weight-regular`, `-light`, `-medium` ou
  `-bold` sem um fallback explícito (`var(--token, 400)`) até o Figma ser
  corrigido — ver causa raiz abaixo
- **note (causa raiz):** na library do Figma, a collection `02. theme
  tokens` nomeou os aliases de `light`/`regular`/`medium`/`bold` com o
  **mesmo nome** do primitivo em `01. base tokens` (só `xlight`/`sbold`/
  `xbold` escaparam disso). Ao achatar para CSS, o alias sobrescreve o
  primitivo e vira uma referência circular
  (`--font-weight-regular: var(--font-weight-regular);`), que o navegador
  trata como valor inválido — como `font-weight` é herdável, o elemento
  **herda o peso do pai** em vez de usar 400/500/700. `scripts/sync-tokens.mjs`
  já falha o build quando essa colisão acontece (ver `GLOSSARIO_PAPEIS_TEXTO.md`
  não afetado, mas `TIPOGRAFIA.md` documenta os valores como se resolvessem
  corretamente — está desatualizado até a correção no Figma ser reexportada).
- **impacto real no código hoje** (checado via grep, não é só teórico):
  - **Mitigado** com fallback explícito: `Button.module.css:19` e
    `Chip.module.css:20` (`var(--button-font-weight, 500)` /
    `var(--chip-font-weight, 500)`), e todos os usos em
    `InputSelect.module.css`
  - **Não mitigado** (sem fallback, herda peso do elemento pai
    silenciosamente): `InputText.module.css:184,218`,
    `InputSearch.module.css:84,232`, `InputPassword.module.css:180,262` —
    todos consomem `--text-placeholder-text-font-weight` ou
    `--text-support-text-font-weight`, ambos quebrados
  - **Seguros por acidente:** todo uso de `--text-label-font-weight`
    (Checkbox, InputText, InputSearch, InputPassword) — esse token
    resolve via `sbold`, que não está na lista de quebrados
- **pairsWith:** ao adicionar fallback, use o valor numérico que o
  primitivo correspondente tem em `src/tokens/base.json` (`light`=300,
  `regular`=400, `medium`=500, `bold`=700), não um número arbitrário
...
| `breadcrumb-*` | Trilha de navegação hierárquica | Zona B (top bar) do template ERP, indicando profundidade | **Note:** o link é `font-weight-bold` (700, hoje quebrado — ver 3.3) e o item atual é `font-weight-regular` (400, também quebrado) — inversão em relação à maioria dos DS (que deixam o atual em destaque); ambos precisam de fallback numérico até o Figma ser corrigido |
